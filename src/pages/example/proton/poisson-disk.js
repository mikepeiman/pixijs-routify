...
export function PoissonDiskSampler( width, height, minDistance, sampleFrequency ){
	this.width = width;
	this.height = height;
...
PoissonDiskSampler.prototype.sample = function(){
	...
	for ( var i = 0; i < this.sampleFrequency; i++ ){
		let samplePoint = this.grid.randomPointAround( processPoint );
...
RandomQueue.prototype.pop = function(){
	...
	let randomIndex = getRandomInt( 0, this.queue.length );
	...
	let element = this.queue[ randomIndex ];
